...
/// The `EditScanViewController` offers an interface for the user to edit the
...
final class EditScanViewController: UIViewController {
  ...
  /// The detected quadrilateral that can be edited by the user. Uses the
  /// image's coordinates.
  ...
  override public func viewWillDisappear(_ animated: Bool) {
    ...
    // Work around for an iOS 11.2 bug where UIBarButtonItems don't get back to
    // their normal state after being pressed.
  ...
  private func setupConstraints() {
    ...
    quadViewWidthConstraint = quadView.widthAnchor
      .constraint(equalToConstant: 0.0)
    quadViewHeightConstraint = quadView.heightAnchor
      .constraint(equalToConstant: 0.0)
  ...
  @objc func pushReviewController() {
    ...
    let orientedImage = ciImage
      .oriented(forExifOrientation: Int32(cgOrientation.rawValue))
    let scaledQuad = quad.scale(quadView.bounds.size, image.size)
    self.quad = scaledQuad

    // Cropped Image
    var cartesianScaledQuad = scaledQuad
      .toCartesian(withHeight: image.size.height)
  ...
  /// The quadView should be lined up on top of the actual image displayed by
  /// the imageView.
  /// Since there is no way to know the size of that image before run time, we
  ...
  /// Generates a `Quadrilateral` object that's centered and 90% of the size of
  /// the passed in image.
